...
<body>
	...
	<div class="container-fluid">
		<div class="page-content page-content-popup">
			...
			if (isset($_GET['bid']) && isset($_GET['status'])) {
				$bid = $_GET['bid'];
				$status = $_GET['status'];
				$result = $book->statusBook($bid, $status);
			}
			?>
											...
											if ($getBooks != false) { $i=1;
												while ($row = $getBooks->fetch_assoc()) {
													...
													<tr>
														...
														<td>
															<?php if ($row['status'] == 1) { ?>
																<a href="book-list.php?bid=<?php echo $row['id'].'&status=0'; ?>" class="btn btn-outline btn-circle btn-sm blue">
																	<i class="fa fa-eye"></i> Идэвхитэй
																</a>
															<?php } else { ?>
																<a href="book-list.php?bid=<?php echo $row['id'].'&status=1'; ?>" class="btn btn-outline btn-circle btn-sm red">
